...
async function criarUsuario(req, res, next) {
  try {
    const { user, senhaTemporaria } = await userService.criarUsuario(req.body);
...
async function listarUsuarios(req, res, next) {
  try {
    ...
    res.status(200).json(await userService.listarUsuarios(query));
  } catch (err) {
    next(err);
  }
}
...
async function atualizarUsuario(req, res, next) {
  try {
    const user = await userService.atualizarUsuario(req.user.id, req.params.id, req.body);
...
async function resetarSenha(req, res, next) {
  try {
    const { user, senhaTemporaria } = await userService.resetarSenha(req.params.id);
